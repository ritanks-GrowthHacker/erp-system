...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const poSummary = await erpDb.execute(sql`
      SELECT 
    ...
    // RFQ statistics
    const rfqSummary = await erpDb.execute(sql`
      SELECT 
        COUNT(*) as total_rfqs,
        COUNT(CASE WHEN status = 'draft' THEN 1 END) as draft_count,
    ...
    const purchaseTrends = await erpDb.execute(sql`
      SELECT 
    ...
    const categorySpending = await erpDb.execute(sql`
      SELECT 
    ...
    const invoiceSummary = await erpDb.execute(sql`
      SELECT 
    ...
    const deliveryPerformance = await erpDb.execute(sql`
      SELECT 
    ...
    const pendingReceipts = await erpDb.execute(sql`
      SELECT 
        po.id,
        po.po_number,
